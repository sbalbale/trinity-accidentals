          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {performances.map((perf) => {
              const PerformanceCard = (
                <div
                  className={`bg-card rounded-lg overflow-hidden border border-border hover:border-primary transition-colors flex flex-col md:flex-row ${perf.recordingUrl ? "cursor-pointer" : ""}`}
                >
                  {perf.image && (
              ...
              return <div key={perf._id}>{PerformanceCard}</div>;
            })}
          </div>
        </section>
